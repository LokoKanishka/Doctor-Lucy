...
def main():
    ...
    try:
        init = request(
            ...
            1,
            "initialize",
            {"protocolVersion": "2024-11-05", "capabilities": {}, "clientInfo": {"name": "compat-test", "version": "0.1"}},
        )
        expect(init["result"]["serverInfo"]["name"] == "lucy-fs-readonly", "initialize server name mismatch")
        expect("tools" in init["result"]["capabilities"], "initialize missing tools capability")

        send_message(proc, {"jsonrpc": "2.0", "method": "notifications/initialized", "params": {}})

        ping = request(proc, 2, "ping", {})
        expect(ping["result"] == {}, "ping should return empty object")

        resources = request(proc, 3, "resources/list", {})
        expect(resources["result"]["resources"] == [], "resources/list should be empty")

        prompts = request(proc, 4, "prompts/list", {})
        expect(prompts["result"]["prompts"] == [], "prompts/list should be empty")

        tools = request(proc, 5, "tools/list", {})
        ...
        grep_resp = request(
            ...
            6,
            "tools/call",
            {"name": "lucy_grep_text", "arguments": {"query": "delegate_mission", "path": "scripts", "max_results": 10, "ignored": "x"}},
        )
        ...
        read_resp = request(
            ...
            7,
            "tools/call",
            {"name": "lucy_read_lines", "arguments": {"path": "scripts/lucy_openclaw_bridge.py", "start": 138, "end": 138, "extra": True}},
        )
        ...
        unknown = request(proc, 9, "unknown/method", {"x": 1})
        expect(unknown["error"]["code"] == -32601, "unknown method should return method-not-found")

        print("MCP OpenClaw compat test: OK")
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=5)
